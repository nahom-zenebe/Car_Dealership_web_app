...
export default function UserProfile() {
  ...
  const fetchVerificationRequest = async () => {
    try {
      const response = await fetch(`/api/user/${uservalue.id}/verifications`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleSave = async () => {
    try {
      ...
      if (
        (!uservalue.phone && tempUser.phone) ||
        (!uservalue.address && tempUser.address)
      ) {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const updateResponse = await axios.patch(
          `/api/user/${uservalue.id}/updateinfo`,
          {
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <Badge variant={statusMap[status]?.variant || "outline"}>
      ...
      return (
        <div className="bg-blue-50 text-blue-800 p-4 rounded-md">
          <div className="flex items-center gap-2">
            ...
            <p>
              Complete your profile with phone and address to start verification
            </p>
          </div>
        </div>
      );
        ...
        <div className="bg-green-50 text-green-800 p-4 rounded-md">
          ...
          {verificationRequest.updatedAt && (
            <p className="text-sm mt-2">
              Verified on:{" "}
              {format(
                new Date(verificationRequest.updatedAt),
                "MMM d, yyyy h:mm a"
              )}
            </p>
          )}
      ...
      return (
        <div className="bg-red-50 text-red-800 p-4 rounded-md">
          ...
          {verificationRequest.comments && (
            <p className="text-sm mt-2">
              Reason: {verificationRequest.comments}
            </p>
          )}
        </div>
      );
    ...
    return (
      <div className="bg-gray-50 text-gray-800 p-4 rounded-md">
        <div className="flex items-center gap-2">
          <Shield className="h-5 w-5" />
          <p>
            Verification status will appear here once you add phone and address.
          </p>
        </div>
      </div>
    );
            ...
            <CardContent className="flex flex-col items-center gap-4">
              ...
              {editMode ? (
                ...
                <div className="text-center w-full">
                  ...
                  <div className="my-2">{renderVerificationBadge()}</div>

                  <div className="w-full border-t pt-4 mt-4 space-y-2 text-left">
                    <p className="text-sm">
                      <span className="font-medium">Phone:</span>{" "}
                      {uservalue?.phone || "Not provided"}
                    </p>
                    <p className="text-sm">
                      <span className="font-medium">Address:</span>{" "}
                      {uservalue?.address || "Not provided"}
                    </p>
                    <p className="text-sm">
                      <span className="font-medium">Member since:</span>{" "}
                      {uservalue?.createdAt
                        ? format(new Date(uservalue.createdAt), "MMM d, yyyy")
                        : "N/A"}
                    </p>
                  </div>
              ...
              <Tabs defaultValue="profile" className="w-full">
                ...
                <TabsContent value="payments" className="pt-4">
                  <div className="space-y-4">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {uservalue?.savedPaymentMethods?.length
