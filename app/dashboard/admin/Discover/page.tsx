...
import AdminCarRentalCard from '@/app/components/cards/AdminCarRentalCard';
...
export default function UserDashboard() {
  ...
  const handleEdit = (updatedCar: any) => {
    setCars(prev => prev.map(car => car.id === updatedCar.id ? updatedCar : car));
    setFilteredCars(prev => prev.map(car => car.id === updatedCar.id ? updatedCar : car));
        ...
        {error ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-8">
            {filteredCars.length === 0 && !loading && (
              <div className="col-span-full text-center text-gray-500 py-12">No cars found.</div>
            )}
